...
const allPoems = [
  ...tangshiPoems.map(p => ({ ...p, category: 'tangshi', categoryName: '唐诗', link: `/shici/tangshi/${p.id}` })),
  ...songciPoems.map(p => ({ ...p, category: 'songci', categoryName: '宋词', link: `/shici/songci/${p.id}` })),
  ...yuanquPoems.map(p => ({ ...p, category: 'yuanqu', categoryName: '元曲', link: `/shici/yuanqu/${p.id}` })),
  ...shijingPoems.map(p => ({ ...p, category: 'shijing', categoryName: '诗经', link: `/shici/shijing/${p.id}` })),
  ...chuciPoems.map(p => ({ ...p, category: 'chuci', categoryName: '楚辞', link: `/shici/chuci/${p.id}` })),
  ...hanweishiPoems.map(p => ({ ...p, category: 'hanweishi', categoryName: '汉魏诗', link: `/shici/hanweishi/${p.id}` }))
]

...
function updateCurrentId() {
  const path = window.location.pathname
  const match = path.match(/\/(mingzhu|xiaoshuo|shenmo|gongan)\/([^/]+?)(?:\.html)?$/)
  if (match) {
...
const prevLink = computed(() => {
  ...
  return `/${prevBook.value.category}/${prevBook.value.id}.html`
})

const nextLink = computed(() => {
  if (!nextBook.value) return ''
  return `/${nextBook.value.category}/${nextBook.value.id}.html`
})
</script>
